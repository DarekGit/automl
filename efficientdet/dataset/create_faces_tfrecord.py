...
#modified /content/automl/efficientdet/dataset/create_faces_tfrecord.py

"""Convert Faces_DD dataset to TFRecord.

Example usage:
    python create_faces_tfrecord.py  --data_dir=  --output_path=/tmp/faces
"""
import hashlib
import io
import json
...
flags.DEFINE_integer('num_shards', 25, 'Number of shards for output file.')
...
def dict_to_tf_example(data,
  ...
  width = int(data['ori_size'][0])
  height = int(data['ori_size'][1])
...
def main(_):
  ...
  annotations_dir = os.path.join(data_dir, FLAGS.annotations_dir, 'Annotations.json')
  ...
  logging.info('Reading from Faces_DD dataset.')
  for idx, example in enumerate(Faces_DD):
    if FLAGS.num_images and idx >= FLAGS.num_images:
      break
    if idx % 100 == 0:
      logging.info('On image %d of %d', idx, len(Faces_DD))
